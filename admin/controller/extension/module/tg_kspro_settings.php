...
class ControllerExtensionModuleTgKSproSettings extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if(isset($_POST['button-active'])){
	            ...
	            $this->response->redirect($this->url->link('extension/module/tg_kspro_settings&submit=true', 'user_token=' . $this->session->data['user_token'], true));
            }
		}
			...
			if(isset($_POST['button-save'])){
				if(is_writable(DIR_CATALOG . 'view/theme/kingstorepro/skins') && is_writable(DIR_CATALOG . 'view/theme/kingstorepro/skins/store_'.$data['edit_skin_store']) && is_writable(DIR_CATALOG . 'view/theme/kingstorepro/skins/store_'.$data['edit_skin_store'].'/'.$data['active_skin_edit'])) {
					...
					if(file_exists(DIR_CATALOG . 'view/theme/kingstorepro/skins/store_'.$data['edit_skin_store'].'/') && filetype(DIR_CATALOG . 'view/theme/kingstorepro/skins/store_'.$data['edit_skin_store'].'/') == 'dir' && file_exists(DIR_CATALOG . 'view/theme/kingstorepro/skins/store_'.$data['edit_skin_store'].'/'.$data['active_skin_edit'].'')) {
						...
						$this->session->data['success'] = $this->language->get('text_success');
						$this->response->redirect($this->url->link('extension/module/tg_kspro_settings&submit=true&skin_edit=' . $data['active_skin_edit'] . '', 'user_token=' . $this->session->data['user_token'], true));
					}
				}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {	
			...
			if(isset($_POST['install_tg_kspro_all_module'])){
				...
				$this->session->data['success'] = $this->language->get('text_success');
				$this->response->redirect($this->url->link('extension/module/tg_kspro_settings', 'user_token=' . $this->session->data['user_token'], true));
		...
		$this->model_setting_setting->deleteSetting( "newsletter");
					...
					// INFORMATION INSTALL
					$output["modulel_information_status"] = 1; 
					$this->model_setting_setting->editSetting( "module_information", $output );
					$query = $this->db->query("
							INSERT INTO `".DB_PREFIX."layout_module` (`layout_module_id`, `layout_id`, `code`, `position`, `sort_order`) VALUES
							(NULL, 1, 'information', 'column_right', 0);
		...
		$data['action'] = $this->url->link('extension/module/tg_kspro_settings', 'user_token=' . $this->session->data['user_token'], true);
	...
	public function install() {
		...
		$this->model_setting_extension->uninstall('module', 'account');
		$this->model_setting_extension->install('module', 'tg_kspro_revolution_slider');
		$this->model_setting_extension->install('module', 'tg_kspro_megamenu');
		$this->model_setting_extension->install('module', 'tg_kspro_megamenu_vertical');
		$this->model_setting_extension->install('module', 'tg_kspro_filter_product');
		...
		$this->model_setting_extension->install('module', 'html');
		
		$this->model_setting_extension->install('module', 'ncategory');
		$this->model_setting_extension->install('module', 'news');
		$this->model_setting_extension->install('module', 'newsletter');
...
$this->model_setting_setting->editSettingValue('theme_default', 'theme_default_image_thumb_width', 400);	
	...
	public function uninstall() {
		$this->load->model('setting/extension');
		$this->load->model('setting/setting');
		$this->model_setting_extension->uninstall('module', 'tg_kspro_revolution_slider');
		$this->model_setting_extension->uninstall('module', 'tg_kspro_megamenu');
		$this->model_setting_extension->uninstall('module', 'tg_kspro_megamenu_vertical');
		$this->model_setting_extension->uninstall('module', 'tg_kspro_filter_product');
		$this->model_setting_extension->uninstall('module', 'account');
		...
		$this->model_setting_extension->uninstall('module', 'ncategory');
		$this->model_setting_extension->uninstall('module', 'news');
		$this->model_setting_extension->uninstall('module', 'newsletter');
